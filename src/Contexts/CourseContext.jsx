...
import {
  initCourseState,
  reducer,
} from '../Reducers/CourseReducer/reducer';
import {
  getAll,
  addCourse,
  deleteCourse,
...
export const CourseProvider = (prop) => {
    ...
    const [openFormDialogEditCourse, setOpenFormDialogEditCourse] = useState(false);
    const [openFormDialogDeleteCourse, setOpenFormDialogDeleteCourse] = useState(false);
    ...
    const handleGetCourseById = useCallback(async (courseId) => {
      try {
    ...
    const handleCreateCourse = useCallback(async (course, setProgressValue) => {
      try {
        // Bắt đầu upload, set giá trị tiến trình là 0
        setProgressValue(0);
    
        const response = await courseApi.createCourse(course, (progressEvent) => {
    ...
    const handleUpdateCourse = useCallback(async (courseId, course, setProgressValue) => {
      try {
        // Bắt đầu upload, set giá trị tiến trình là 0
        setProgressValue(0);
    
        const response = await courseApi.updateCourse(courseId, course, (progressEvent) => {
          // Xử lý tiến trình upload ở đây
          const progress = Math.round((progressEvent.loaded / progressEvent.total) * 100);
          setProgressValue(progress);
        });
