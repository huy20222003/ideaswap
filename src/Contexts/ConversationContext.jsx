...
export const ConversationProvider = (prop) => {
    const [conversationState, dispatch] = useReducer(reducer, initStateConversation);
    const [openFormDeleteConversation, setOpenFormDeleteConversation] = useState(false);
    ...
    const handleGetConversationsByUserId = useCallback(async (userId) => {
      try {
    ...
    const handleGetOneConversations = useCallback(async (conversationID) => {
      try {
        const response = await conversationApi.getOneConversation(conversationID);
        if (response.data.success) {
    ...
    const handleAddConversation = useCallback(async (data) => {
      try {
